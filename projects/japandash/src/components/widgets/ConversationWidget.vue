<template>
  <WidgetFrame title="Conversation" icon="💬" widget-id="conversation" collapsible :loading="conv.loading.value">
    <template #default="{ focused }">

      <!-- Compact summary (dashboard tile) -->
      <div v-if="!focused" class="space-y-3">
        <div v-if="!conv.hasKey.value" class="text-center py-4 text-usuzumi text-sm space-y-1">
          <p class="text-2xl">🔑</p>
          <p>Add API key in Settings to start</p>
        </div>
        <template v-else>
          <div class="rounded-lg bg-koshi/20 p-3 space-y-1">
            <p class="font-mono text-[0.55rem] uppercase tracking-widest text-usuzumi">Scenario</p>
            <p class="text-sm font-medium">{{ conv.activeScenario.value.label }}</p>
            <template v-if="lastAssistantLine">
              <p class="text-xs text-usuzumi mt-2 line-clamp-2 italic">「{{ lastAssistantLine }}」</p>
            </template>
            <p v-else class="text-xs text-usuzumi mt-2">Type below to start the conversation.</p>
          </div>
          <p class="text-[0.65rem] text-usuzumi font-mono">
            {{ conv.activeHistory.value.length }} turn{{ conv.activeHistory.value.length !== 1 ? 's' : '' }}
          </p>
        </template>
      </div>
        ...
        <template v-else>
          ...
          <div class="flex items-center gap-2">
            <select
              ...
              class="flex-1 px-2 py-1.5 text-xs rounded-md border border-koshi bg-surface/80 text-sumi focus:outline-none focus:ring-1 focus:ring-ai/40"
          ...
          <!-- Message log -->
          <div ref="logEl" class="max-h-[55vh] overflow-y-auto space-y-3 py-1 pr-1">
...
const lastAssistantLine = computed(() => {
