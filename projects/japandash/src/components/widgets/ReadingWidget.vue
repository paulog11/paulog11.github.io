<template>
  <WidgetFrame title="Reading Practice" icon="📖" widget-id="reading" collapsible>
      ...
      <div v-if="!focused" class="space-y-3">
        <div class="rounded-lg bg-koshi/20 p-4">
          <div class="flex items-start justify-between gap-2 mb-1">
            <h3 class="font-display text-xl font-semibold leading-tight">{{ currentPassage.title }}</h3>
            <span class="flex-shrink-0 px-1.5 py-0.5 text-[0.55rem] rounded bg-ai-light text-ai font-medium">
              {{ currentPassage.level }}
            </span>
          </div>
          <p class="text-xs text-usuzumi">{{ currentPassage.titleEn }}</p>
        </div>
        ...
        <details>
          ...
          <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-1">
            <div
              ...
              :key="v.word"
              class="flex items-baseline gap-2 px-2 py-1 rounded bg-surface/50 text-sm"
            >
        ...
        <details>
          ...
          <div class="mt-2 space-y-2">
            <div
              ...
              :key="i"
              class="rounded-md bg-surface/50 px-3 py-2"
            >
