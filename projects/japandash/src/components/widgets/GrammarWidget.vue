<template>
  <WidgetFrame title="Grammar" icon="文" widget-id="grammar" collapsible>
    <template #default="{ focused }">

      <!-- Compact summary (dashboard tile) -->
      <div v-if="!focused" class="space-y-3">
        <div v-if="currentPoint" class="rounded-lg bg-koshi/20 p-4 text-center">
          <p class="font-mono text-[0.55rem] uppercase tracking-widest text-usuzumi mb-2">Today's Grammar</p>
          <p class="text-3xl font-display font-semibold text-ai mb-1">{{ currentPoint.pattern }}</p>
          <p class="text-sm text-sumi">{{ currentPoint.meaning }}</p>
          <div class="flex items-center justify-center gap-2 mt-2">
            <span class="px-1.5 py-0.5 text-[0.55rem] rounded bg-ai-light text-ai font-medium">{{ selectedLevel }}</span>
            <span v-if="studiedPoints.includes(currentPoint.id)" class="text-[0.65rem] text-matcha">✓ Studied</span>
          </div>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        <div v-if="currentPoint" class="rounded-lg bg-koshi/30 p-4">
          ...
          <div class="mt-3">
            ...
            <ul class="space-y-0.5">
              <li
                ...
                :key="i"
                class="text-xs font-mono bg-surface/50 rounded px-2 py-1"
              >
                {{ f }}
              </li>
            </ul>
          ...
          <div class="mt-3">
            ...
            <div
              ...
              :key="i"
              class="bg-surface/50 rounded-md px-3 py-2 mb-1.5"
            >
        ...
        <!-- Browse list -->
        <div v-if="browseMode" class="max-h-[40vh] overflow-y-auto space-y-1 pr-1">
          <div
