<template>
  <WidgetFrame title="Onomatopoeia" icon="💥" widget-id="onomatopoeia" collapsible>
    <template #default="{ focused }">

      <!-- Compact summary (dashboard tile) -->
      <div v-if="!focused" class="space-y-3">
        <div v-if="currentWord" class="rounded-lg bg-koshi/20 p-4 text-center">
          <p class="font-mono text-[0.55rem] uppercase tracking-widest text-usuzumi mb-2">Today's Word</p>
          ...
          <p class="text-sm font-medium mt-2">{{ currentWord.meaning }}</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        <div v-if="currentWord" class="rounded-lg bg-koshi/30 p-4 text-center">
          ...
          <!-- Example -->
          <div v-if="currentWord.examples.length > 0" class="mt-3 text-left bg-surface/50 rounded-md p-2">
...
const jumpedFrom = ref(null)
const jumpedFromIndex = ref(null)
